...
<head>
    ...
    <link rel="stylesheet" href="{{asset('bootstrapp/css/bootstrap-grid.min.css')}}">
    <link rel="stylesheet" href="{{asset('bootstrapp/css/bootstrap-grid.css.map')}}">
    <title>
    ...
    <script src="{{asset('bootstrapp/js/bootstrap.min.js')}}"></script>
    <script src="{{asset('bootstrapp/js/bootstrap.bundle.js')}}"></script>
    <script src="{{asset('bootstrapp/js/bootstrap.js')}}"></script>
...
<body class="bg-gray-100">
        ...
        <div class="container mx-auto px-4 md:flex md:items-center md:gap-0">
            ...
            <div class="grow justify-between navbar-collapse" :class="{ 'flex flex-col': !isCollapsed, 'hidden md:flex': isCollapsed }">
                ...
                <ul class="flex flex-col md:flex-row gap-3">
                    ...
                    @auth
                        <li>
                            <a class="text-gray-500" href="{{ route('forum.unread') }}">{{ trans('Discussion non lus mis à jour | ') }}</a>
                        </li>
                    @endauth
                    @can ('moveCategories')
                        <li>
                            <a class="text-gray-500" href="{{ route('forum.category.manage') }}">{{ trans('Forum Général Gérer | ') }}</a>
                        </li>
                    @endcan
                </ul>
    ...
    // Ajoutez un écouteur d'événements à chaque bouton d'onglet
    tabButtons.forEach(function(button) {
      button.addEventListener('click', function(event) {
        ...
        // Activez uniquement le bouton d'onglet actuel
        this.classList.add('active');

        // Vous pouvez ajouter ici le code pour afficher le contenu de l'onglet correspondant si nécessaire
      });
    });
  });
    </script>
